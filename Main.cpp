...
namespace VendingMachine {
	...
	int checkDrinkInput(void) {
		std::string input = VendingMachine::getUserInputLine();
		...
		drinkId = VendingMachine::getDrinkFromNumberName(std::string_view(input));
		...
		for (int i = 0; i < VendingMachine::NUM_DRINKS; i++) {
			...
			// Match!:
			for (const auto &drTok : drinkTokens) {
				numMatchedTokens = 0; // Re-initialize.

				for (const auto &inTok : inputTokens)
		...
		for (const auto &[drId, numMatchedTokens] : drinkIdToMatchedTokensMap)
			if (numMatchedTokens > maxMatches) {
				maxMatches = numMatchedTokens;
				maxMatchedDrinkId = drId;
			}

		return maxMatchedDrinkId;
	}

	int getDrinkFromNumberName(const std::string_view p_input) {
		for (int i = 0; i < VendingMachine::NUM_DRINKS; i++)
	...
	int getDrinkFromNumericalSymbol(const std::string &p_input) {
		try {
